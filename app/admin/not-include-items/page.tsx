...
interface NotIncludeItem {
  id?: string;
  ko: string;
  en: string;
}

export default function AdminNotIncludeItemsPage() {
  const [items, setItems] = useState<NotIncludeItem[]>([]);
  ...
  const fetchItems = async () => {
    ...
    const data = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() } as NotIncludeItem));
    setItems(data);
    setLoading(false);
  };
  ...
  const handleAdd = async () => {
    ...
    await addDoc(collection(db, "notIncludeItems"), {
      ko: newKo.trim(),
      en: newEn.trim(),
  ...
  // 편집 시작
  const startEdit = (item: NotIncludeItem) => {
  ...
  const handleEditSave = async (id: string) => {
    ...
    await updateDoc(doc(db, "notIncludeItems", id), {
      ko: editKo.trim(),
      en: editEn.trim(),
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6">불포함사항 관리</h1>
      {/* 추가 폼 */}
      <div className="flex flex-col sm:flex-row gap-2 mb-6">
        <input
          type="text"
          placeholder="불포함사항 (한국어)"
          value={newKo}
        ...
        <input
          type="text"
          placeholder="Not included item (English)"
          value={newEn}
