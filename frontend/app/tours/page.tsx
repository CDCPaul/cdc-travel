              ...
              >
                <Link href={`/tours/${product.id}`} className="flex bg-white rounded-xl shadow p-6 flex-col hover:shadow-lg transition">
                  {typeof product.imageUrl === 'string' && product.imageUrl ? (
                    <Image src={product.imageUrl} alt={product.title || 'Tour Image'} width={600} height={400} className="rounded mb-4 object-cover" />
                  ) : null}
                  <h3 className="text-2xl font-semibold mb-2">{product.title}</h3>
                  {/* <div className="text-lg mb-1">{product.date}</div> */}
                  <div className="text-xl font-bold text-teal-700 mb-2">{product.price}</div>
